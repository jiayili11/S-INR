...
p.add_argument('--data_path', type=str, default="./datas/noise/")
p.add_argument('--lr', type=float, default=1e-5)
p.add_argument('--epochs', type=int, default=1001)
...
for dataname in ['lehavim']:
    for case in ['case3', 'case4']:
        for lr in [5e-4]:  # 1e-5, 5e-5, 1e-4, 5e-4, 1e-3
            ...
            data_test = data_load_img_noise_overlap_bound(data_path, gt_path, pixel_set=pixel_set, freq=freq)
            dataloader = DataLoader(data_test, batch_size=1, pin_memory=False, num_workers=0)
            datas = next(iter(dataloader))
            img_gt = data_test.img_gt
            img_ob = data_test.img_ob
            ...
            net = NET_S_INR(pixel_num, c_in=c_in, r=r, c_out=img_gt.shape[2], c_mid=cmid,
                            hidden_layers=h - 2,
            ...
            for iteration in range(opt.epochs):
                ...
                for j in range(pixel_num):
                    loss += torch.norm(out[j] - gt_ob[j], 2)
                if iteration % 50 == 0 and iteration != 0:
                    ...
                    if iteration % 3000 == 0 and iteration != 0:
                        print(dataname, case, omega, "loss", loss.item(), "psnr",
                              Psnr_gt, "time", time.time() - t1)
                    if iteration == opt.epochs - 1:
                        ...
                        print(dataname, case, lr, omega, "loss", loss.item(), "psnr_best",
